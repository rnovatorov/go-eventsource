...
func (s *Saga) TaskArguments(id string) *structpb.Struct {
	...
	args, _ := structpb.NewStruct(nil)

	for _, arg := range t.definition.Arguments {
		switch v := arg.Value.(type) {
		case *sagapb.ArgumentDefinition_StaticValue:
			args.Fields[arg.Name] = v.StaticValue
		case *sagapb.ArgumentDefinition_ResultReference:
			...
			if ref := arg.GetResultReference(); ref != "" {
				if !graph.dependsOn(t.Id, ref) {
					return nil, fmt.Errorf("%s: %s: %w: %s",
						t.Id, arg.Name, ErrTaskArgumentReferencesIndependentResult, ref)
				}
			}
		}
		...
		for _, arg := range t.CompensationArguments {
			...
			if ref := arg.GetResultReference(); ref != "" {
				if ref != t.Id && !graph.dependsOn(t.Id, ref) {
					return nil, fmt.Errorf("%s: %s: %w: %s",
						t.Id, arg.Name, ErrTaskCompensationArgumentReferencesIndependentResult, ref)
				}
			}
		}
	}
...
func (s *Saga) applySagaBegun(sc *sagapb.SagaBegun) {
	...
	s.dependencyGraph, _ = buildDependencyGraph(sc.TaskDefinitions)
}
